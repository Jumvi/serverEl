  ...
  auth: {
   ...
   pass: "kerenemvi95" 
    ...
    try {
      ...
      // Configuration du email de vérification
      const mailOptions = {
        from: 'judahmvi@gmail.com',
        to: email,
        subject: 'Aidez-nous à vérifier votre compte',
      ...
      transporter.sendMail(mailOptions, (error, info) => {
        if (error) {
          console.error('Erreur lors de l\'envoie du mail de vérification:', error);
          return res.status(500).json({ success: false, message: 'erreur envoie mail' });
        }
